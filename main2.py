...
img_obj1 = PhotoImage(file="image/bg.png")
sc.create_image(0, 0, anchor=NW, image=img_obj1)

img_obj2 = PhotoImage(file="image/bg2.png")
...
figures = [[[x + W // 2, y + 1, 1, 1] for x, y in figures_pos] for figures_pos in figures_pos]
...
sc.create_text(505, 30, text="TETRIS", font=("WiGuru 2", 40), fill="red", anchor=NW)
...
for item in grid:
    sc.itemconfig(item, fill=rgb_to_hex(get_color()))
...
game_sc.bind_all("<KeyPress-Up>", move_object)
game_sc.bind_all("<KeyPress-Down>", move_object)
game_sc.bind_all("<KeyPress-Left>", move_object)
game_sc.bind_all("<KeyPress-Right>", move_object)
